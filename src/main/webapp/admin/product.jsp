                                        ...
                                        <c:if test="${info.pageNum==i}">
                                            <li>
                                                ...
                                                <a href="javascript:ajaxsplit(${i})"
                                                   style="background-color: forestgreen">${i}</a>
                                            </li>
                                        </c:if>
                                        <c:if test="${info.pageNum!=i}">
                                            <li>
...
<script type="text/javascript">
            ...
            if (confirm("您确定删除" + zhi.length + "条商品吗？")) {
                ...
                $.ajax({
                    ...
                    success:function(msg){
                        alert(msg);//执行语句
                        //将页面上显示商品的容器重新加载
                        $("#table").load("http://localhost:8080/MMSM/admin/product.jsp #table");
    ...
    function del(pid,pageNum) {
        ...
        if (confirm("你确定删除吗?")) {
            ...
            var minPrice = $("#lprice").val();
            var maxPrice = $("#hprice").val();
            //发出ajax请求,进行删除操作
            $.ajax({
                url: "${pageContext.request.contextPath}/prod/delete.action",
                data: {"pid": pid,"pageNum":pageNum,"pName":pName,"typeId":typeId,"minPrice":minPrice,"maxPrice":maxPrice},
                type: "post",
                dateType: "text",
                success: function (msg) {
                    alert(msg);
    ...
    function one(pid, ispage) {
        ...
        var minPrice = $("#lprice").val();
        var maxPrice = $("#hprice").val();
        var str="&pid="+pid+"&pageNum="+ispage+"&pName="+pName+"&typeId="+typeId+"&minPrice="+minPrice+"&maxPrice"+maxPrice;
        location.href = "${pageContext.request.contextPath}/prod/getOne.action?"+str;
    }
</script>
<!--分页的AJAX实现-->
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        $.ajax({
            ...
            data: {"pageNum": page == 0 ? 1 : page,"pName":pName,"typeId":typeId,"minPrice":minPrice,"maxPrice":maxPrice},
    ...
    function condition(){
        ...
        $.ajax({
            type:"post",
            url:"${pageContext.request.contextPath}/prod/ajaxsplit.action",
            data:{"pName":pName,"typeId":typeId,"minPrice":minPrice,"maxPrice":maxPrice},
            success:function () {
                // alert("hello world!!")
                //刷新显示数据的容器
                $("#table").load("http://localhost:8080/MMSM/admin/product.jsp #table");
